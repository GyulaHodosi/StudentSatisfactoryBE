...
using StudentSatisfactoryBackend.Repositories.NewsRepository;
using StudentSatisfactoryBackend.Repositories.NewsRepository.Interfaces;
using StudentSatisfactoryBackend.Repositories.ReportRepository;
    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddCors(options =>
            {
                ...
                builder =>
                {
                    builder.WithOrigins("https://codecool-satisfactionapp.netlify.app").AllowAnyMethod().AllowAnyHeader().AllowCredentials();
                });
            });
            services.ConfigureIISIntegration();

            services.AddControllers();
            services.AddEntityFrameworkNpgsql().AddDbContext<SurveyContext>(options =>
            {
                ...
                if (env == "Development")
                {
                    ...
                    connStr = Configuration.GetConnectionString("SurveyConnection");
                }
                else
                {
            ...
            .AddGoogle(options =>
                {
                    ...
                    if (env == "Production")
                    {
                        ClientData = new ClientData { ClientId = Environment.GetEnvironmentVariable("CLIENT_ID"), ClientSecret = Environment.GetEnvironmentVariable("CLIENT_SECRET") };
                    }
                    else
                    {
